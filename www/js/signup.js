...
$(document).ready(function() {
    $("#loginForm").submit(function (event) {
        ...
        postOption = regex.test(username) ? {
            email: username,
        ...
        var posting = $.post(url, postOption);
    ...
    $("#registerForm").submit(function (event) {
        event.preventDefault();
        var $form = $(this);
        var username = $form.find('input[name="usernamesignup"]').val();
        var email = $form.find('input[name="emailsignup"]').val();
        var password = $form.find('input[name="passwordsignup"]').val();
        var password_confirm = $form.find('input[name="passwordsignup_confirm"]').val();
        if (password !== password_confirm) {
            alert("Input passwords do not match");
            return;
        }
        var url = $form.attr('action');
        var posting = $.post(url, {username: username, email: email, password: md5(password) });
        posting.done(function (data) {
            if (data) {
                console.log(data);
                try {
